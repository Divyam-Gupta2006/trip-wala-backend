...
class MembershipsService {
    repo = new memberships_repository_1.MembershipsRepository();
    // --- Applications Business Logic ---
    async applyToTrip(tripId, userId, message) {
        ...
        if (!trip || trip.isDeleted) {
            throw new errors_1.ApiError(404, 'TRIP_NOT_FOUND', 'The trip you are applying to does not exist.');
        }
        if (trip.status === 'cancelled' || trip.status === 'completed') {
            throw new errors_1.ApiError(400, 'INVALID_TRIP_STATE', 'Cannot apply to a cancelled or completed trip.');
        }
        // 2. Check if already a member
        const isMember = trip.members.some((m) => m.userId === userId);
        if (isMember) {
            throw new errors_1.ApiError(400, 'ALREADY_MEMBER', 'You are already a member of this trip.');
        }
        ...
        if (existing && (existing.status === 'pending' || existing.status === 'accepted')) {
            throw new errors_1.ApiError(400, 'DUPLICATE_APPLICATION', 'You have already applied to this trip.');
        }
        // 4. Check capacity
        if (trip.members.length >= trip.maxMembers) {
            throw new errors_1.ApiError(400, 'TRIP_FULL', 'This trip is already full.');
        }
        ...
        await Promise.all(organizers.map((org) => notifService.publish({
            userId: org.userId,
            actorId: userId,
    ...
    async cancelApplication(applicationId, userId) {
        ...
        if (app.userId !== userId) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_APPLICATION_ACCESS', 'You can only cancel your own application.');
        }
        if (app.status !== 'pending') {
            throw new errors_1.ApiError(400, 'INVALID_STATE_TRANSITION', 'Can only cancel pending applications.');
        }
        return this.repo.updateApplicationStatus(applicationId, 'cancelled');
    }
    async acceptApplication(applicationId, organizerId, reviewNotes) {
        const app = await this.repo.findApplicationById(applicationId);
        if (!app) {
            throw new errors_1.ApiError(404, 'APPLICATION_NOT_FOUND', 'Application not found.');
        }
        ...
        if (!organizerMember || (organizerMember.role !== 'organizer' && organizerMember.role !== 'coOrganizer')) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'Only trip organizers can review applications.');
        }
        if (app.status !== 'pending') {
            throw new errors_1.ApiError(400, 'INVALID_STATE_TRANSITION', 'Can only accept pending applications.');
        }
        // Check capacity
        if (app.trip.members.length >= app.trip.maxMembers) {
            throw new errors_1.ApiError(400, 'TRIP_FULL', 'Trip capacity has already been reached.');
        }
    ...
    async rejectApplication(applicationId, organizerId, reviewNotes) {
        const app = await this.repo.findApplicationById(applicationId);
        if (!app) {
            throw new errors_1.ApiError(404, 'APPLICATION_NOT_FOUND', 'Application not found.');
        }
        ...
        if (app.status !== 'pending') {
            throw new errors_1.ApiError(400, 'INVALID_STATE_TRANSITION', 'Can only reject pending applications.');
        }
    ...
    async inviteUser(tripId, inviterId, inviteeId, role = 'member') {
        const trip = await db_1.prisma.trip.findUnique({
            where: { id: tripId },
            include: { members: true },
        });
        if (!trip || trip.isDeleted) {
            throw new errors_1.ApiError(404, 'TRIP_NOT_FOUND', 'The requested trip does not exist.');
        }
        ...
        if (!inviterMember || (inviterMember.role !== 'organizer' && inviterMember.role !== 'coOrganizer')) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'Only organizers can invite members to this trip.');
        }
        // Check if invitee is already a member
        const isMember = trip.members.some((m) => m.userId === inviteeId);
        if (isMember) {
            throw new errors_1.ApiError(400, 'ALREADY_MEMBER', 'User is already a member of this trip.');
        }
        // Check for existing pending invitation
        const existing = await this.repo.findInvitationByTripAndUser(tripId, inviteeId);
        if (existing && existing.status === 'pending') {
            throw new errors_1.ApiError(400, 'DUPLICATE_INVITATION', 'User has already been invited to this trip.');
        }
        // Check capacity
        if (trip.members.length >= trip.maxMembers) {
            throw new errors_1.ApiError(400, 'TRIP_FULL', 'Cannot send invitation as the trip is already full.');
        }
    ...
    async cancelInvitation(invitationId, organizerId) {
        const invitation = await this.repo.findInvitationById(invitationId);
        if (!invitation) {
            throw new errors_1.ApiError(404, 'INVITATION_NOT_FOUND', 'Invitation not found.');
        }
        const organizerMember = invitation.trip.members.find((m) => m.userId === organizerId);
        if (!organizerMember || (organizerMember.role !== 'organizer' && organizerMember.role !== 'coOrganizer')) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'Only organizers can cancel invitations.');
        }
        if (invitation.status !== 'pending') {
            throw new errors_1.ApiError(400, 'INVALID_STATE_TRANSITION', 'Can only cancel pending invitations.');
        }
        return this.repo.updateInvitationStatus(invitationId, 'cancelled');
    }
    async acceptInvitation(invitationId, inviteeId) {
        const invitation = await this.repo.findInvitationById(invitationId);
        if (!invitation) {
            throw new errors_1.ApiError(404, 'INVITATION_NOT_FOUND', 'Invitation not found.');
        }
        if (invitation.inviteeId !== inviteeId) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_INVITATION_ACCESS', 'You can only respond to invitations sent to you.');
        }
        if (invitation.status !== 'pending') {
            throw new errors_1.ApiError(400, 'INVALID_STATE_TRANSITION', 'Can only accept pending invitations.');
        }
        // Check capacity
        if (invitation.trip.members.length >= invitation.trip.maxMembers) {
            throw new errors_1.ApiError(400, 'TRIP_FULL', 'Cannot accept invitation as the trip is full.');
        }
    ...
    async declineInvitation(invitationId, inviteeId) {
        const invitation = await this.repo.findInvitationById(invitationId);
        if (!invitation) {
            throw new errors_1.ApiError(404, 'INVITATION_NOT_FOUND', 'Invitation not found.');
        }
        ...
        if (invitation.status !== 'pending') {
            throw new errors_1.ApiError(400, 'INVALID_STATE_TRANSITION', 'Can only decline pending invitations.');
        }
    ...
    async addMemberDirectly(tripId, organizerId, userId, role = 'member') {
        const trip = await db_1.prisma.trip.findUnique({
            where: { id: tripId },
            include: { members: true },
        });
        if (!trip || trip.isDeleted) {
            throw new errors_1.ApiError(404, 'TRIP_NOT_FOUND', 'The trip does not exist.');
        }
        ...
        if (!orgMember || (orgMember.role !== 'organizer' && orgMember.role !== 'coOrganizer')) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'Only organizers can add members directly.');
        }
        ...
        // Capacity check
        if (trip.members.length >= trip.maxMembers) {
            throw new errors_1.ApiError(400, 'TRIP_FULL', 'Trip capacity has been reached.');
        }
        return db_1.prisma.$transaction(async (tx) => {
    ...
    async removeMember(tripId, organizerId, userId) {
        const trip = await db_1.prisma.trip.findUnique({
            where: { id: tripId },
            include: { members: true },
        });
        ...
        const orgMember = trip.members.find((m) => m.userId === organizerId);
        if (!orgMember || (orgMember.role !== 'organizer' && orgMember.role !== 'coOrganizer')) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'Only organizers can remove members.');
        }
        // Verify user is in trip
        const memberToRemove = trip.members.find((m) => m.userId === userId);
        if (!memberToRemove) {
            throw new errors_1.ApiError(404, 'MEMBER_NOT_FOUND', 'User is not a member of this trip.');
        }
        ...
        if (memberToRemove.role === 'organizer') {
            ...
            if (otherOrganizers.length === 0) {
                throw new errors_1.ApiError(400, 'SOLE_ORGANIZER_REMOVE_ERROR', 'Cannot remove the sole organizer of a trip.');
            }
        }
        return db_1.prisma.$transaction(async (tx) => {
    ...
    async leaveTrip(tripId, userId) {
        ...
        if (!member) {
            throw new errors_1.ApiError(404, 'MEMBER_NOT_FOUND', 'You are not a member of this trip.');
        }
        // Sole organizer cannot leave
        if (member.role === 'organizer') {
            const otherOrganizers = trip.members.filter((m) => m.role === 'organizer' && m.userId !== userId);
            if (otherOrganizers.length === 0) {
                throw new errors_1.ApiError(400, 'SOLE_ORGANIZER_LEAVE_ERROR', 'The sole organizer cannot leave the trip before transferring ownership.');
            }
        }
        return db_1.prisma.$transaction(async (tx) => {
    ...
    async promoteMember(tripId, organizerId, userId, role) {
        const trip = await db_1.prisma.trip.findUnique({
            where: { id: tripId },
            include: { members: true },
        });
        ...
        if (!orgMember || orgMember.role !== 'organizer') {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'Only the primary organizer can promote members or transfer ownership.');
        }
        const targetMember = trip.members.find((m) => m.userId === userId);
        if (!targetMember) {
            throw new errors_1.ApiError(404, 'MEMBER_NOT_FOUND', 'Target user is not a member of this trip.');
        }
