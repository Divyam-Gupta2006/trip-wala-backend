...
class MessagingService {
    repo = new messaging_repository_1.MessagingRepository();
    // --- Auth Helper ---
    async verifyAccess(conversationId, userId) {
        const conversation = await this.repo.getConversationById(conversationId);
        if (!conversation) {
            throw new errors_1.ApiError(404, 'CONVERSATION_NOT_FOUND', 'Conversation not found.');
        }
        if (conversation.type === 'trip') {
            // For trip chats, verify the user is an active member of the trip
            const tripId = conversation.tripId;
            const membership = await db_1.prisma.tripMember.findUnique({
                where: { tripId_userId: { tripId, userId } },
            });
            if (!membership) {
                throw new errors_1.ApiError(403, 'FORBIDDEN_CHAT_ACCESS', 'You must be a member of the trip to access this chat.');
            }
        }
        else {
            ...
            if (!isParticipant) {
                throw new errors_1.ApiError(403, 'FORBIDDEN_CHAT_ACCESS', 'You are not a participant in this conversation.');
            }
        }
        return conversation;
    }
    ...
    async getOrCreateDirectConversation(userId, targetUserId) {
        if (userId === targetUserId) {
            throw new errors_1.ApiError(400, 'INVALID_TARGET_USER', 'You cannot start a direct message with yourself.');
        }
        ...
        if (!targetUser || targetUser.isDeleted) {
            throw new errors_1.ApiError(404, 'USER_NOT_FOUND', 'The target user does not exist or has deleted their account.');
        }
        ...
        if (!newConv) {
            throw new errors_1.ApiError(500, 'DATABASE_ERROR', 'Failed to create direct conversation.');
        }
    ...
    async getConversations(userId) {
        ...
        const formatted = await Promise.all(list.map((c) => this.formatConversation(c, userId)));
        return formatted;
    }
    ...
    async getMessageHistory(conversationId, userId, limit, cursor) {
        await this.verifyAccess(conversationId, userId);
    ...
    async sendMessage(conversationId, senderId, data) {
        ...
        if (!sender) {
            throw new errors_1.ApiError(404, 'USER_NOT_FOUND', 'Sender not found.');
        }
    ...
    async editMessage(messageId, userId, text) {
        ...
        if (!message) {
            throw new errors_1.ApiError(404, 'MESSAGE_NOT_FOUND', 'Message not found.');
        }
        if (message.senderId !== userId) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_MESSAGE_EDIT', 'You can only edit your own messages.');
        }
        if (message.isDeleted) {
            throw new errors_1.ApiError(400, 'INVALID_MESSAGE_STATE', 'Cannot edit a deleted message.');
        }
    ...
    async deleteMessage(messageId, userId) {
        ...
        if (message.senderId !== userId) {
            throw new errors_1.ApiError(403, 'FORBIDDEN_MESSAGE_DELETE', 'You can only delete your own messages.');
        }
    ...
    async toggleReaction(messageId, userId, emoji) {
        ...
        if (message.isDeleted) {
            throw new errors_1.ApiError(400, 'INVALID_MESSAGE_STATE', 'Cannot react to a deleted message.');
        }
        const updated = await this.repo.toggleReaction(messageId, userId, emoji);
        if (!updated) {
            throw new errors_1.ApiError(500, 'DATABASE_ERROR', 'Failed to update emoji reaction.');
        }
        return this.formatMessage(updated);
    }
    // --- Mark Read ---
    ...
    // --- Formatters ---
    async formatConversation(conv, currentUserId) {
        let name = conv.name || '';
    ...
    formatMessage(msg) {
        ...
        let parsedMentions = [];
        try {
            parsedMentions = typeof msg.mentions === 'string' ? JSON.parse(msg.mentions) : (msg.mentions || []);
        }
        catch {
            parsedMentions = [];
        }
