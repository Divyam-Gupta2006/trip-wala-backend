...
exports.formatTripResponse = formatTripResponse;
...
async function createTrip(req, res, next) {
    try {
        ...
        const newTrip = await db_1.prisma.$transaction(async (tx) => {
            // Create trip record
        ...
        if (!newTrip) {
            throw new errors_1.ApiError(500, 'DATABASE_ERROR', 'Failed to create trip and retrieve records.');
        }
...
async function getTripById(req, res, next) {
    try {
        ...
        if (!trip || trip.isDeleted) {
            throw new errors_1.ApiError(404, 'TRIP_NOT_FOUND', 'The requested trip does not exist or has been deleted.');
        }
        // Authorization: Check private trip visibility
        if (trip.visibility === 'private') {
            const isMember = trip.members.some(m => m.userId === currentUserId);
            if (!isMember) {
                throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_ACCESS', 'You do not have permission to view this private trip.');
            }
        }
...
async function updateTrip(req, res, next) {
    try {
        ...
        if (userRole !== 'organizer') {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_UPDATE', 'Only the trip organizer can modify trip details.');
        }
        ...
        if (updateData.status && updateData.status !== trip.status) {
            ...
            let isValid = false;
            if (current === 'draft' && (target === 'open' || target === 'cancelled'))
                isValid = true;
            else if (current === 'open' && (target === 'full' || target === 'inProgress' || target === 'completed' || target === 'cancelled'))
                isValid = true;
            else if (current === 'full' && (target === 'open' || target === 'inProgress' || target === 'completed' || target === 'cancelled'))
                isValid = true;
            else if (current === 'inProgress' && (target === 'completed' || target === 'cancelled'))
                isValid = true;
            if (!isValid) {
                throw new errors_1.ApiError(400, 'INVALID_STATUS_TRANSITION', `Cannot change trip status from '${current}' to '${target}'.`);
            }
        }
...
async function deleteTrip(req, res, next) {
    try {
        ...
        const userRole = trip.members.find(m => m.userId === currentUserId)?.role;
        if (userRole !== 'organizer') {
            throw new errors_1.ApiError(403, 'FORBIDDEN_TRIP_DELETE', 'Only the trip organizer can delete this trip.');
        }
...
async function searchTrips(req, res, next) {
    try {
        ...
        const rawTrips = await db_1.prisma.trip.findMany({
            where,
            orderBy,
            include: {
                members: {
...
async function getUserTrips(req, res, next) {
    try {
        ...
        if (!userExists || userExists.isDeleted) {
            throw new errors_1.ApiError(404, 'USER_NOT_FOUND', 'User does not exist.');
        }
